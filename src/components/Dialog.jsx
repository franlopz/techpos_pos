...
const Dialog = ({
  ...
  const CloseIcon = () => {
    return (
      <span className="relative -top-3 -right-5">
        <button
    ...
    <div className="fixed inset-0 z-50 overflow-auto bg-smoke-light flex 
      ...
      <div className="fixed z-10 inset-0 overflow-y-auto">
        <div className="flex items-center justify-center min-h-screen pt-4
          ...
          <div
            className={`inline-block max-w-lg align-bottom bg-white rounded-lg text-left 
                overflow-hidden shadow-xl transform sm:my-8 
                sm:align-middle`}
            role="dialog"
          >
            <div className="relative px-8 pt-5 bg-white w-full flex justify-between flex-row
                rounded-lg">
              <div className="text-xl font-semibold">{title}</div>
              {CloseIcon()}
            </div>
            <div className="bg-white">
              <div className={`bg-white rounded px-8 pt-6 pb-8 flex flex-col overflow-y-auto ${size}`}>
                {children}
              </div>
            </div>
            {actionsPannel}
